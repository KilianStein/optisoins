...
export class FeuilleSoinsResolve implements Resolve<IFeuilleSoins> {
    ...
    resolve(route: ActivatedRouteSnapshot, state: RouterStateSnapshot): Observable<FeuilleSoins> {
        const id = route.params['id'] ? route.params['id'] : null;
        if (id) {
            return this.service.find(id).pipe(
                filter((response: HttpResponse<FeuilleSoins>) => response.ok),
                map((feuilleSoins: HttpResponse<FeuilleSoins>) => feuilleSoins.body)
            );
        }
