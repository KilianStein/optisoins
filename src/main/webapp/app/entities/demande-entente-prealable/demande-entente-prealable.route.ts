...
export class DemandeEntentePrealableResolve implements Resolve<IDemandeEntentePrealable> {
    ...
    resolve(route: ActivatedRouteSnapshot, state: RouterStateSnapshot): Observable<DemandeEntentePrealable> {
        const id = route.params['id'] ? route.params['id'] : null;
        if (id) {
            return this.service.find(id).pipe(
                filter((response: HttpResponse<DemandeEntentePrealable>) => response.ok),
                map((demandeEntentePrealable: HttpResponse<DemandeEntentePrealable>) => demandeEntentePrealable.body)
            );
        }
