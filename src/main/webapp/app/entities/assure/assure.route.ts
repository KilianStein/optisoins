...
export class AssureResolve implements Resolve<IAssure> {
    ...
    resolve(route: ActivatedRouteSnapshot, state: RouterStateSnapshot): Observable<Assure> {
        const id = route.params['id'] ? route.params['id'] : null;
        if (id) {
            return this.service.find(id).pipe(
                filter((response: HttpResponse<Assure>) => response.ok),
                map((assure: HttpResponse<Assure>) => assure.body)
            );
        }
