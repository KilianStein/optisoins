...
export class RemplacanteResolve implements Resolve<IRemplacante> {
    ...
    resolve(route: ActivatedRouteSnapshot, state: RouterStateSnapshot): Observable<Remplacante> {
        const id = route.params['id'] ? route.params['id'] : null;
        if (id) {
            return this.service.find(id).pipe(
                filter((response: HttpResponse<Remplacante>) => response.ok),
                map((remplacante: HttpResponse<Remplacante>) => remplacante.body)
            );
        }
