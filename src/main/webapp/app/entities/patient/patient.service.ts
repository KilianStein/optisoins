...
export class PatientService {
    ...
    private convertDateFromClient(patient: IPatient): IPatient {
        const copy: IPatient = Object.assign({}, patient, {
            dateNaissance:
                patient.dateNaissance != null && patient.dateNaissance.isValid() ? patient.dateNaissance.format(DATE_FORMAT) : null
        });
        return copy;
    }
    ...
    private convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
        res.body.forEach((patient: IPatient) => {
            patient.dateNaissance = patient.dateNaissance != null ? moment(patient.dateNaissance) : null;
        });
        return res;
    }
}
