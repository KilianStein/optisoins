...
export class CarteAideMedicaleResolve implements Resolve<ICarteAideMedicale> {
    ...
    resolve(route: ActivatedRouteSnapshot, state: RouterStateSnapshot): Observable<CarteAideMedicale> {
        const id = route.params['id'] ? route.params['id'] : null;
        if (id) {
            return this.service.find(id).pipe(
                filter((response: HttpResponse<CarteAideMedicale>) => response.ok),
                map((carteAideMedicale: HttpResponse<CarteAideMedicale>) => carteAideMedicale.body)
            );
        }
