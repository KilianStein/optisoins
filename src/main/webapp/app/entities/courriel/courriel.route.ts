...
export class CourrielResolve implements Resolve<ICourriel> {
    ...
    resolve(route: ActivatedRouteSnapshot, state: RouterStateSnapshot): Observable<Courriel> {
        const id = route.params['id'] ? route.params['id'] : null;
        if (id) {
            return this.service.find(id).pipe(
                filter((response: HttpResponse<Courriel>) => response.ok),
                map((courriel: HttpResponse<Courriel>) => courriel.body)
            );
        }
